...
export default function PlanAnalyticsView({ healthData, bottleneckData, utilizationData }: PlanAnalyticsViewProps) {
  if (!healthData) return <div className="p-8 text-red-500">No analytics data available.</div>;
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="rounded-lg border bg-white p-6 flex flex-col gap-2">
          ...
          <div className="text-xs text-muted-foreground">
            {healthData.totalBudget.used} used, {healthData.totalBudget.remaining} remaining
          </div>
        </div>
        <div className="rounded-lg border bg-white p-6 flex flex-col gap-2">
          <div className="flex items-center gap-2">
          ...
          <div className="text-xs text-muted-foreground">
            {healthData.totalAssignedTeachers} teachers assigned
          </div>
        </div>
        <div className="rounded-lg border bg-white p-6 flex flex-col gap-2">
          <div className="flex items-center gap-2">
          ...
          <div className="text-xs text-muted-foreground">
            {healthData.totalAssignedTeachers}/{healthData.totalRequiredTeachers} filled
          </div>
        </div>
        <div className="rounded-lg border bg-white p-6 flex flex-col gap-2">
          <div className="flex items-center gap-2">
          ...
          <div className="text-2xl font-bold">
            {healthData.isBudgetCompliant ? "Compliant" : "Warning"}
          </div>
          <div className="text-xs text-muted-foreground">
            {healthData.isBudgetCompliant ? "Within budget" : healthData.complianceWarning}
          </div>
        </div>
      </div>
      ...
      <Tabs defaultValue="health" className="w-full">
        <TabsList className="grid w-full grid-cols-3 lg:w-[600px]">
          <TabsTrigger value="health">Plan Health & Budget</TabsTrigger>
          <TabsTrigger value="bottlenecks">Subject Bottlenecks</TabsTrigger>
          <TabsTrigger value="utilization">Teacher Utilization</TabsTrigger>
        </TabsList>
...
function HealthView({ data }: { data: AllocationHealthReportDto }) {
  const budgetData = [
    { name: 'Used', value: data.totalBudget.used, fill: '#3b82f6' },
    { name: 'Remaining', value: data.totalBudget.remaining, fill: '#e2e8f0' },
  ];

  const splitData = [
    { name: 'Elementary', allocated: data.elementaryBudget.allocated, used: data.elementaryBudget.used },
    { name: 'Middle School', allocated: data.middleSchoolBudget.allocated, used: data.middleSchoolBudget.used },
  ];
    ...
    <div className="space-y-8">
      {!data.isBudgetCompliant && (
        <div className="rounded-lg border border-destructive/50 bg-destructive/5 p-4 flex items-center gap-4 text-destructive">
          ...
          <div>
            <h3 className="font-semibold">Budget Compliance Warning</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="rounded-lg border bg-white p-4 flex flex-col items-center">
          <div className="font-semibold mb-2">Total Budget</div>
        ...
        <div className="rounded-lg border bg-white p-4 md:col-span-2">
          <div className="font-semibold mb-2">Budget Split Analysis</div>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={splitData} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
              ...
              <Legend />
              <Bar dataKey="allocated" name="Limit" fill="#94a3b8" barSize={20} radius={[0, 4, 4, 0]} />
              <Bar dataKey="used" name="Used" fill="#3b82f6" barSize={20} radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="rounded-lg border bg-white p-4 flex flex-col gap-2">
          <div className="font-semibold mb-2">Demand Fulfillment</div>
          <div className="text-2xl font-bold">{data.fulfillmentPercentage.toFixed(1)}%</div>
          <p className="text-xs text-muted-foreground mb-2">of required positions filled</p>
          <div className="space-y-1 text-sm">
            ...
            <div className="flex justify-between">
              <span className="text-muted-foreground">Teachers Required:</span>
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="rounded-lg border bg-white p-4 lg:col-span-2">
          <div className="font-semibold mb-2">Top 10 Supply Gaps</div>
          <ResponsiveContainer width="100%" height={300}>
            <ComposedChart data={chartData} layout="vertical" margin={{ top: 20, right: 20, bottom: 20, left: 40 }}>
              ...
              <RechartsTooltip />
              <Legend />
              <Bar dataKey="availableTeacherCount" name="Available Supply" barSize={20} fill="#94a3b8" radius={[0, 4, 4, 0]} />
              <Bar dataKey="requiredTeacherCount" name="Required Demand" barSize={20} fill="#ef4444" radius={[0, 4, 4, 0]} />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="rounded-lg border bg-white p-4 lg:col-span-1">
          <div className="font-semibold mb-2 text-destructive flex items-center gap-2">
            <AlertCircle className="h-5 w-5" /> Critical Shortages
          </div>
              ...
              {data.filter(i => i.status === "CRITICAL_SHORTAGE" || i.status === "SHORTAGE").map((item, idx) => (
                <div key={idx} className="flex flex-col p-3 border rounded-lg bg-red-50/50 border-red-100">
                  <div className="flex justify-between items-start">
                    ...
                    <Badge variant="destructive" className="text-[10px]">{item.gap} Gap</Badge>
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">
                    {item.schoolType} • Needs {item.requiredTeacherCount} / Has {item.availableTeacherCount}
                  </div>
                  <div className="mt-2 text-xs font-medium text-red-600">
                    Suggestion: Increase group size per teacher.
                  </div>
                </div>
              ))}
...
function UtilizationView({ data }: { data: TeacherUtilizationReportDto[] }) {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(t => t.utilizationStatus === filter);
  }, [data, filter]);

  const stats = useMemo(() => ({
    unused: data.filter(t => t.utilizationStatus === "UNUSED").length,
    over: data.filter(t => t.utilizationStatus === "OVER_UTILIZED").length,
    under: data.filter(t => t.utilizationStatus === "UNDER_UTILIZED").length,
  }), [data]);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="p-4 border rounded-lg bg-red-50 border-red-100 text-center">
          ...
          <div className="text-xs text-red-800 font-medium">Unused Teachers</div>
        </div>
        <div className="p-4 border rounded-lg bg-yellow-50 border-yellow-100 text-center">
          ...
          <div className="text-xs text-yellow-800 font-medium">Under-Utilized (1 Assign)</div>
        </div>
        <div className="p-4 border rounded-lg bg-blue-50 border-blue-100 text-center">
          ...
          <div className="text-xs text-blue-800 font-medium">Over-Utilized ({'>'}2 Assign)</div>
        </div>
      </div>
      ...
      <div className="rounded-lg border bg-white p-4">
        <div className="flex flex-row items-center justify-between pb-2">
          <div>
            <div className="font-semibold">Teacher Utilization Details</div>
            <div className="text-xs text-muted-foreground">Monitor efficiency and credit balance</div>
          </div>
          <Select value={filter} onValueChange={setFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="OVER_UTILIZED">Over Utilized</SelectItem>
            </SelectContent>
          </Select>
        </div>
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="text-right w-24">
                    ...
                    <div className={`font-bold text-sm ${teacher.currentCreditBalance < 0 ? 'text-red-600' : 'text-green-600'}`}>
                      {teacher.currentCreditBalance > 0 ? '+' : ''}{teacher.currentCreditBalance} hrs
                    </div>
                  </div>
                  <Badge variant="outline" className={`w-24 justify-center ${
                    ...
                    {teacher.utilizationStatus.replace('_', ' ')}
                  </Badge>
                </div>
              </div>
            ))}
            {filteredData.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">No teachers match this filter.</div>
            )}
          </div>
        </ScrollArea>
      </div>
